...
function SchemeUserPage({
  ...
  const SavingsInvestment = schemeUserData.investments?.filter(
    (item) => item.type === "NonRefundable"
  );

  // Calculate total
  const totalFD = schemeUserData.investments
    ?.filter((item) => item.type === "FD")
    ?.reduce((acc, item) => acc + item.amount, 0);
  const totalInv = schemeUserData.investments
    ?.filter((item) => item.type === "NonRefundable")
    ?.reduce((acc, item) => acc + item.amount, 0);
  ...
  return (
    <div className="user-scheme-container">
      ...
      <div className="user-scheme-details-cover">
        ...
        <div className="user-scheme-table-cover">
          <div
            className="user-scheme-table-wrapper"
            style={{
              overflowY: "scroll",
            }}
          >
            <div>
              <div>
                <h4 className="table-header">
                  Investment History{" "}
                  <span className="header-period">Period :Life Time</span>
                </h4>
                {FDinvestment?.length > 0 ? (
                  <table
                    style={{
                      overflowY: "scroll",
                    }}
              ...
              <div style={{ marginTop: "15px" }}>
                ...
                {SavingsInvestment?.length > 0 ? (
                  ...
                  >
                    ...
                    <tbody>
                      {SavingsInvestment?.map((item, index) => (
                        <tr key={item._id}>
                          <td>
                            {item.date &&
                              format(
